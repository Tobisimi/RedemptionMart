-- RedemptionMart V1: Orders (spec Section 5)
-- Payment (Paystack) added in a later migration; orders start as unpaid + pending.
...
-- Buyers see their own orders; sellers see orders for their shop.
...
-- Atomically place an order: validates products, snapshots prices, creates items.
...
BEGIN
  ...
  -- Validate all products and compute total before writing anything.
